...
export async function approveBooking(input: unknown) {
    const user = await requireUser();

    if (user.role !== "SPECIALIST") {
        throw new Error("Only specialists can approve bookings");
    }
    ...
    if (!updated) {
        throw new Error(
            "Booking not found, not yours, or already processed"
        );
    }
